...
template <typename inPair_t, typename outData_t, typename Callable_t> struct Dispatcher;

template <typename inPair_t, typename outData_t, typename Callable_t> class impl_Request {
    Callable_t _func;
    Dispatcher<inPair_t, outData_t, Callable_t> *_dispatcher;

  public:
    const inPair_t &_data;
    impl_Request(const Callable_t &func, const inPair_t &v, Dispatcher<inPair_t, outData_t, Callable_t> *dispatcher) : _data(v), _func(func), _dispatcher(dispatcher) {}

    template <typename P_t> void process(const P_t &p) {
        if constexpr (std::is_same_v<typename P_t::first_type, impl_KEY>) { //Note: cppcheck has a bug here. This is not a syntax error
            _dispatcher->threadSafe_push_outData(std::make_pair(p.first, _func(p.second)));
        } else {
            _dispatcher->threadSafe_push_outData(_func(p));
        }
    }
};

template <typename inPair_t, typename outData_t, typename Callable_t> class impl_Worker {
    impl_Request<inPair_t, outData_t, Callable_t> *request = nullptr;
    bool running = true;
    Dispatcher<inPair_t, outData_t, Callable_t> *_dispatcher;

  public:
    explicit impl_Worker(Dispatcher<inPair_t, outData_t, Callable_t> *dispatcher) : _dispatcher(dispatcher) {}
    void run() {
        while (running) {
            if (_dispatcher->addWorker(this)) {
                request->process(request->_data);
            } else {
                running = false;
            }
        }
    ...
    void receive_request(impl_Request<inPair_t, outData_t, Callable_t> *request) { this->request = request; }
};
...
template <typename inPair_t, typename outData_t, typename Callable_t> struct Dispatcher {
    using Key_t = std::decay_t<typename inPair_t::first_type>;

    template <typename Input_t, typename Output_t, typename L> Dispatcher(Input_t &&input, Output_t &output, L &&lambda, const size_t &numThreads) noexcept : numRequests(input.size()) {
        outData.clear();
        reserve_sizes(numRequests);
        set_concurrent_tasks(input, lambda);
  ...
  private:
    ...
    std::vector<impl_Request<inPair_t, outData_t, Callable_t> *> requests;
    std::vector<impl_Request<inPair_t, outData_t, Callable_t> *> requests_backup; // backup for deleting
    std::mutex mu_requests;
    std::vector<impl_Worker<inPair_t, outData_t, Callable_t> *> allWorkers;
    ...
    bool start_threads(const size_t &workers) {
        impl_Worker<inPair_t, outData_t, Callable_t> *w = nullptr;
        for (size_t i = 0; i < workers; ++i) {
            w = new impl_Worker<inPair_t, outData_t, Callable_t>(this);
            allWorkers.push_back(w);
            threads.emplace_back(&impl_Worker<inPair_t, outData_t, Callable_t>::run, w);
        }
        return true;
    }
    ...
    template <typename Input_t, typename L> void set_concurrent_tasks(const Input_t &inData, L &&func) {
        impl_Request<inPair_t, outData_t, Callable_t> *rq = nullptr;
        for (const auto &ele : inData) {
            // Every request has its own copy of lambda
            rq = new impl_Request<inPair_t, outData_t, Callable_t>(func, ele, this);
            requests.push_back(rq);
            requests_backup.push_back(rq);
        }
    }

    bool addWorker(impl_Worker<inPair_t, outData_t, Callable_t> *worker) {
    ...
    template <typename P_t> void threadSafe_push_outData(P_t &&data) {
        std::scoped_lock lk(mu_outData);
        outData.push_back(std::forward<P_t>(data));
        if (outData.size() == numRequests) {
            cv_mainThread.notify_one();
        }
    }

    template <typename Output_t> void move_out(Output_t &output) {
    ...
    friend void impl_Worker<inPair_t, outData_t, Callable_t>::run();
    template <typename P_t> friend void impl_Request<inPair_t, outData_t, Callable_t>::process(const P_t &p);
};
} // namespace Dispatcher
...
template <typename Input_t, typename Output_t, typename Callable_t> void run_concurrent_tasks(const Input_t &input, Output_t &output, const size_t &numThreads, Callable_t &&lambda) {
    ...
    using inPair_t = typename Input_t::value_type;
    ...
    output.clear();
    if constexpr (!std::is_same_v<Output_t, std::map<Key_t, inData_t>>) output.reserve(input.size());
    Dispatcher::Dispatcher<inPair_t, outData_t, Callable_t> dispatcher(input, output, lambda, numThreads);
}
...
template <typename Input_t, typename Output_t, typename Callable_t> void run_concurrent_tasks_ordered(Input_t input, Output_t &output, const size_t &numThreads, Callable_t &&lambda) {
    ...
    using inPair_t = std::pair<Key_t, inData_t>;
    ...
    Dispatcher::Dispatcher<inPair_t, outData_t, Callable_t> dispatcher(std::move(concurrent_input), concurrent_output, lambda, numThreads);
